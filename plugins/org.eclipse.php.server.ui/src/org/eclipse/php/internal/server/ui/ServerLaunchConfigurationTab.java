...
public class ServerLaunchConfigurationTab extends
	...
	private void selectDefaultServer(ILaunchConfiguration configuration)
		...
		server = ServersManager.getDefaultServer(project);
	}
	...
	public void performApply(ILaunchConfigurationWorkingCopy configuration) {
		...
		if (resource != null && resource.exists()) {
			server = ServersManager.getDefaultServer(resource.getProject());
		}
		if (server != null) {
		...
		try {
			...
			if (!serverName.equals("")) { //$NON-NLS-1$
				...
				if (server == null) {
					...
					return false;
				} else if (ServersManager.isEmptyServer(server)) {
					setErrorMessage(NLS
							.bind(PHPServerUIMessages
									.getString("ServerLaunchConfigurationTab.NoServerForProject"), //$NON-NLS-1$
