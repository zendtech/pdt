...
public class MoveUtils {
	...
	/**
	 * For a file that is being moved, get the string of an include statement in
	 * it and returns what the new include string should be based on the new
	 * location
	 * 
	 * @param sourceFile
	 ...
	 */
	public static String getMovedIncludedString(IFile sourceFile,
			IPath destinationDirectoryPath, String value,
		...
		if (projectPath.isPrefixOf(destinationDirectoryPath)) {
			...
			for (IResource resouce : selectedResource) {
				if (resouce instanceof IFolder
						&& resouce.getProjectRelativePath().isPrefixOf(
			...
			IPath destIncludedFilePath = sourceFile.getParent().getFullPath()
					.append(value);

			for (IResource resouce : selectedResource) {
				if (resouce instanceof IFile
						&& resouce.getFullPath().equals(destIncludedFilePath)) {
					// if the parent folder is moving, append the folder name to
					// the fileName.
			...
			for (IResource resouce : selectedResource) {
				if (resouce instanceof IFolder
						&& resouce.getProjectRelativePath().isPrefixOf(
								includingPath)) {
					destIncludedFilePath = destinationDirectoryPath.append(
							resouce.getName()).append(fileName);
					break;
				}
			}
	...
	private static boolean isRelativeAndExistingResource(IContainer container,
			String value) {
	 ...
	 * their include statements to it based on the new location. This method
	 * gets the moved file, the current including file we are handling and the
	 * current include statement in it, and returns the new include statement
	 * 
	 * @param selectedResource
	 * 
	 */
	public static String getMovedIncludingString(IFile sourceFile,
			IPath destinationDirectoryPath, IFile includingFile, String value,
			IResource[] selectedResource) {
		return getMovedIncludingString(sourceFile,
				getDirectoryPath(sourceFile.getFullPath()),
				destinationDirectoryPath, includingFile, value,
				selectedResource);
	}

	public static String getMovedIncludingString(IFile sourceFile,
			IPath oldDirectoryPath, IPath destinationDirectoryPath,
			IFile includingFile, String value, IResource[] selectedResource) {
		IPath projectPath = sourceFile.getProject().getFullPath();
		...
		if (projectPath.isPrefixOf(destinationDirectoryPath)) {
			...
			IPath includeFilePath = includingFile.getParent()
					.getProjectRelativePath().append(includePath);

			// Check if the parent folder is moving.
			for (IResource resouce : selectedResource) {
				if (resouce instanceof IFolder
						&& ((resouce.getProjectRelativePath()
								.isPrefixOf(includePath)) || resouce
								.getProjectRelativePath().isPrefixOf(
										includeFilePath))
			...
			for (IResource resouce : selectedResource) {
				...
				if (resouce.getProjectRelativePath().isPrefixOf(includingPath)) {
					// if the parent folder is moving, append the folder name to
					// the fileName.
					includePath = destinationDirectoryPath.append(includePath
							.makeRelativeTo(resouce.getProjectRelativePath()));
					break;
				}
			...
			if (value.startsWith("..") || value.startsWith(".")) { //$NON-NLS-1$ //$NON-NLS-2$
				return path.makeRelativeTo(includingPath).toOSString()
				...
				if (isInSaveProject) {
					...
					return path.makeRelativeTo(projectPath).toOSString()
							.replaceAll("\\\\", "/"); //$NON-NLS-1$ //$NON-NLS-2$
				}
			}
			...
			IPath path = destinationDirectoryPath.append(oldDirectory).append(
					fileName);
			IPath destPath = destinationDirectoryPath;

			if (destPath.isPrefixOf(path)) {
				if (value.startsWith("..") || value.startsWith(".")) { //$NON-NLS-1$ //$NON-NLS-2$
					IPath includingPath = includingFile.getParent()
							.getProjectRelativePath();
					return path.makeRelativeTo(destPath.append(includingPath))
							.toString().replaceAll("\\\\", "/"); //$NON-NLS-1$ //$NON-NLS-2$
				} else {
					return path.makeRelativeTo(destPath).toOSString()
							.replaceAll("\\\\", "/"); //$NON-NLS-1$ //$NON-NLS-2$
				}
			}
	...
	public static Collection<Node> getReferencingFiles(
